...
// nothing.
{
  ...
  const holderSrc = readNormalized(join(repoRoot, "bin", "supervise-holder.sh"));
  ...
  // never by text, and only its operator is removed.
  {
    ...
    if (plusLines.length < 2) {
      ...
      expectRefusal("a += clause rewritten as a plain assignment", "[instruction-reassign]", ["COORDINATOR_ROLE_INSTRUCTION", "bin/supervise-holder.sh line"], () => buildLedgerFrom(shSrc, lines.join("\n"), tsSrc));
    }
  }
  ...
  // position, never by matching text the rule under test was handed.
  {
    ...
    if (plain.length < 1) {
      ...
      expectRefusal("a PRIMING_BODY clause appended with +=", "[priming-shape]", ["PRIMING_BODY"], () => buildLedgerFrom(shSrc, lines.join("\n"), tsSrc));
    }
  }
  ...
  expectRefusal("a renamed *_INSTRUCTION variable", "[instruction-set]", ["LEAD_INSTRUCTION", "COORDINATOR_STEER_INSTRUCTION"], () =>
    buildLedgerFrom(shSrc, holderSrc.split("COORDINATOR_STEER_INSTRUCTION").join("LEAD_INSTRUCTION"), tsSrc));
  ...
  expectRefusal("a variable added to the priming write", "[priming-write]", ["STEWARD_CHARTER"], () =>
    buildLedgerFrom(shSrc, mutated(holderSrc, '$CHANNEL_REPLY_INSTRUCTION" "$PRIMING_BODY"', '$CHANNEL_REPLY_INSTRUCTION$STEWARD_CHARTER" "$PRIMING_BODY"', "priming write"), tsSrc));
  // A seventh context block pushed under a name no rule sizes.
  expectRefusal("a new context block", "[context-blocks]", ["fleetBlock"], () =>
    buildLedgerFrom(shSrc, holderSrc, mutated(tsSrc, "contextBlocks.push(memoryBlock);", "contextBlocks.push(memoryBlock);\n      contextBlocks.push(fleetBlock);", "context block")));
  ...
  expectRefusal("a renamed context block push", "[context-blocks]", ["memoriesBlock", "memoryBlock"], () =>
    buildLedgerFrom(shSrc, holderSrc, mutated(tsSrc, "contextBlocks.push(memoryBlock);", "contextBlocks.push(memoriesBlock);", "renamed push")));
  ...
  expectRefusal("a removed prompt call site", "[prompt-call-sites]", ["expectedNudgeTurn", "no submitExpectedTurn call site"], () =>
    buildLedgerFrom(shSrc, holderSrc, mutated(tsSrc, "const nudgeOutcome = await submitExpectedTurn($, expectedTurns, expectedNudgeTurn);", "const nudgeOutcome = { ok: true };", "removed call site")));
  // A .prompt.submit call outside submitExpectedTurn.
  expectRefusal("a direct .prompt.submit call", "[prompt-submit]", ["found 2"], () =>
    buildLedgerFrom(shSrc, holderSrc, mutated(tsSrc, "result = await dp.prompt.submit({ text: entry.text });", 'result = await dp.prompt.submit({ text: entry.text });\n    await dp.prompt.submit({ text: "again" });', "direct submit")));
  ...
  expectRefusal("a literal inside an excluded delivery site", "[delivery-exclusion]", ["answerText", "Note: "], () =>
    buildLedgerFrom(shSrc, holderSrc, mutated(tsSrc, "deliveryText(answerLabel, answer.id, answer.text,", 'deliveryText(answerLabel, answer.id, "Note: " + answer.text,', "delivery literal")));
  ...
  expectRefusal("an excluded delivery site prefixed outside deliveryText", "[delivery-exclusion]", ["submittedText"], () =>
    buildLedgerFrom(shSrc, holderSrc, mutated(tsSrc, "const submittedText = deliveryText(", 'const submittedText = "[INBOX] " + deliveryText(', "delivery prefix")));
  ...
  expectRefusal("a literal at the delivery site no prompt-call-site row names", "[delivery-exclusion]", ["Note: "], () =>
    buildLedgerFrom(shSrc, holderSrc, mutated(tsSrc, "lines.push(deliveryText(ground, rec.id, rec.text,", 'lines.push(deliveryText(ground, rec.id, "Note: " + rec.text,', "context delivery literal")));
  ...
  expectRefusal("a fourth deliveryText call site", "[delivery-exclusion]", ["found 4"], () =>
    buildLedgerFrom(shSrc, holderSrc, mutated(tsSrc, "lines.push(deliveryText(ground, rec.id, rec.text,", "lines.push(deliveryText(ground, rec.id, rec.text));\n          lines.push(deliveryText(ground, rec.id, rec.text,", "fourth delivery site")));
  ...
  expectRefusal("a nudge-frame piece rewritten to a quoted literal", "[chain-shape]", ["NUDGE_TEXT_idle_gap_converted"], () =>
    buildLedgerFrom(shSrc, holderSrc, mutated(tsSrc, "`The controller read this as an idle gap, not a real fork: no concrete blocking question. `", '"The controller read this as an idle gap, not a real fork: no concrete blocking question. "', "nudge piece requoted")));
  ...
  expectRefusal("a tool description with a hoisted sentence", "[chain-truncated]", ["agentic_resolve"], () =>
    buildLedgerFrom(shSrc, holderSrc, mutated(tsSrc, '        "A reply says a turn answered;', '        SHARED_NOTE +\n        "A reply says a turn answered;', "tool description hoisted")));
  ...
  function expectSizeHeld(label, entryName, find, replacement) {
    ...
    let after;
    try {
      before = buildLedgerFrom(shSrc, holderSrc, tsSrc).find((e) => e.name === entryName);
      after = buildLedgerFrom(shSrc, holderSrc, mutated(tsSrc, find, replacement, label)).find((e) => e.name === entryName);
    } catch (e) {
  ...
  expectRefusal("a delivery text argument spliced from a constant", "[delivery-exclusion]", ["NOTE_PREFIX"], () =>
    buildLedgerFrom(shSrc, holderSrc, mutated(tsSrc, "lines.push(deliveryText(ground, rec.id, rec.text,", "lines.push(deliveryText(ground, rec.id, NOTE_PREFIX + rec.text,", "delivery identifier splice")));
  ...
  expectRefusal("a literal answerTo at the site no named row reaches", "[delivery-exclusion]", ["answerTo"], () =>
    buildLedgerFrom(shSrc, holderSrc, mutated(tsSrc, '{ mark: waited ? "waited" : "urgent" }', '{ mark: waited ? "waited" : "urgent", answerTo: "the standing question" }', "literal answerTo")));
  ...
  try {
    const before = buildLedgerFrom(shSrc, holderSrc, tsSrc).find((e) => e.name === "FLEET_PROMPT_LINE_LITERALS");
    const added = " recorded for this persona";
    const grown = buildLedgerFrom(shSrc, holderSrc, mutated(tsSrc, '"no commons entry"', `"no commons entry${added}"`, "nested label")).find((e) => e.name === "FLEET_PROMPT_LINE_LITERALS");
  ...
  // case. The delimiters are found from the sentence's own position.
  {
    ...
    if (at === -1 || open === -1 || close === -1) {
      ...
      expectRefusal("a fleet note's composed half hoisted into a name", "[fleet-note-prose]", ["REPEATED_NAME_NOTE"], () => buildLedgerFrom(shSrc, holderSrc, hoisted));
    }
  }
  ...
  expectRefusal("a fleet note's composed half split across a +", "[fleet-note-prose]", ["composed half"], () =>
    buildLedgerFrom(shSrc, holderSrc, mutated(tsSrc, noteSentence, "a roster entry repeats a name\" + \" an earlier entry already holds, so it has no row of its own. The name it wrote:", "note split")));
  ...
  // and the pattern matches it on the delimiters around the bare key.
  {
    ...
    if (at === -1 || open === -1 || close === -1 || keyAt === -1) {
      ...
      expectRefusal("a fleet note's composed half in shorthand property form", "[fleet-note-prose]", ["shorthand"], () => buildLedgerFrom(shSrc, holderSrc, shorthand));
    }
  }
  ...
  try {
    const before = buildLedgerFrom(shSrc, holderSrc, tsSrc).find((e) => e.name === "FLEET_NOTE_COMPOSED_PROSE");
    const added = " under the name it repeated";
    const grown = buildLedgerFrom(shSrc, holderSrc, mutated(tsSrc, noteSentence, noteSentence + added, "note lengthened")).find((e) => e.name === "FLEET_NOTE_COMPOSED_PROSE");
  ...
  const reraiseMarker = "[STILL WAITING] Send the question below to the operator";
  {
    ...
    if (at === -1 || open === -1 || close === -1 || close <= open) {
      ...
      expectRefusal("the re-raise instruction hoisted into a name", "[chain-shape]", ["RERAISE_INSTRUCTION"], () => buildLedgerFrom(shSrc, holderSrc, hoisted));
    }
  }
  ...
  try {
    const before = buildLedgerFrom(shSrc, holderSrc, tsSrc).find((e) => e.name === "STILL_WAITING_RERAISE_TEXT");
    const added = " The operator has not replied.";
    const grown = buildLedgerFrom(shSrc, holderSrc, mutated(tsSrc, reraisePhrase, reraisePhrase + added, "re-raise lengthened")).find((e) => e.name === "STILL_WAITING_RERAISE_TEXT");
