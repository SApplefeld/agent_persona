...
The steward's standing instruction carries three duties beyond directing workers. It calls `fleet_status` each tick and reports any persona that is held, backing off, or whose heartbeat has gone stale, once per change of state rather than once per tick. It holds the kit's Coordinator seat and runs that seat's reconciliation pass. And it sends a record that turns on a design decision to the `architect` persona, telling the operator it did so; the architect answers the steward, which relays the answer onward, so the architect never addresses a worker directly.
