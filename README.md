...
When the owner's controller drains the inbox on a quiet tick, it submits the text through `$.prompt.submit` as an `[OPERATOR]` user turn. The `[OPERATOR]` marker is prepended to the text before submission. The plugin keeps an ordered list of the turns its own `$.prompt.submit` calls have queued and not yet opened: a delivery entry carrying its record, a nudge entry, or a plugin entry (the kaizen announcement, the reply backstop, the ask re-raise), each pushed right before its submit and removed if that submit throws. At `turn.start`, an external turn (the real `prompt.submit` hook fired, which a keyboard, SDK or channel turn does and the plugin's own submits never do) leaves the list alone and, where a delivery is queued, records `operator_stamp_withheld` naming `channel-origin` or `external`; the delivery's own turn still stamps it when it opens. Any other turn is the head entry's: a delivery entry stamps its record with the turn id, a nudge or plugin entry stamps nothing, and a turn with the list empty stamps nothing. This rests on the engine opening parked plugin submits in submission order, which its `prompt.submit` contract does not state. When the stamped turn completes with an answer, the controller writes that answer back as the reply and marks the record `answered`. A stamped turn that ends aborted or with an empty answer leaves the record `delivered` with its stamp (`operator_turn_unanswered`); no later turn re-stamps it, and the TTL bounds it.
