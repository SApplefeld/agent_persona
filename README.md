...
- The target is `<coordinatorPersona>` and no live session owns it: only that persona's owner reads the record, so it would wait unread for a coordinator that may never launch. This applies on every ground, fires too in the gap while a coordinator relaunches, and is not counted toward the tool-error streak that pauses a goal
