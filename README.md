...
- Unreadable. A document absent from all four places, one above 256 KiB, or a stored `planPath` that fails the shape check changes nothing and logs one `plan_record_unreadable` decision per entry per session.
...
This deletion also opens a window that stays open until the next step ends the wrapper. The supervisor emits the settings file once, above its relaunch loop (`bin/supervise.sh:342`), and every relaunch passes `--settings` at that same path with no check that the file is still there (`:2560`). So a child that exits and relaunches after the deletion is handed a settings path that no longer exists. What `claude` does with a missing settings file has not been measured on this machine. Keep the gap between this step and the next one short rather than leaving a wrapper running against a deleted file.
...
The two cases take different commands, and the hold marker is what separates them. A persona the roster added was never running, so it carries no marker and takes `Start-ScheduledTask AgentPersona-<name>` on its own. Registering a task does not start it: every `AgentPersona-*` task is triggered at startup (`bin/Register-PersonaTasks.ps1:281`), and the script starts one only under `-Start` (`:446-447`). Do not reach for `-Start` at the first step instead, because that would launch the new personas ahead of the settings deletion and the stops below, which is the order this procedure exists to get right. A persona you stopped through the shutdown tool gave the supervisor a clean exit 0, and the keeper turns exit 0 into a hold (`bin/keeper-functions.ps1:67-72`), so it stays down across every later boot until the marker goes: `Start-Persona.ps1 -Name <name> -Release` removes it, and `Start-ScheduledTask AgentPersona-<name>` then starts the persona again. A persona that went down with a reboot rather than a stop carries no marker and comes back on its own, so read `<rundir>/keeper.hold` to tell the two apart rather than assuming either.
...
1. **Ask wait default:** Whether the owner's ask waits indefinitely for a reply or times out. Default: **60 minutes** (`askOperatorWaitMs` unset, code fallback `hooks/index.ts:242`).
...
Adding a question takes three edits, and none of them is in the journal, which is question-agnostic.
...
The other three kinds belong to the `plan-health` call, and each records something the plugin observed for itself. `lead_blocked` is written at the same turn end, from the same first-line read that sets the lead: whether that turn's closing text opened with `BLOCKED:`. `chapter_within` is written `true` on the first later turn on that entry at which the plan holder's Chapter count stands above the count at the call, which a rise read on a sibling entry's turn also satisfies, and `false` at the fifth turn on the entry without one. `next_speaker` is written at the next turn end, whatever entry that turn was on and even where that turn was skipped, from what opened it. The closing texts and the stamp ids still awaiting an outcome are held in session memory only. A restart, or the entry completing, being abandoned or leaving the tree, drops them, and the outcomes they awaited are never written; a reader of the journal tolerates a call with no outcome.
