...
The reading holds two entries that are not personas: how the roster file itself read, and which of its entries carried a problem. Without them the watcher would go quiet exactly when the fleet stops being watched, since an unreadable roster yields no rows, and no rows yields no persona change. Both are reported when they move and not on every prompt. The reading lives in the persisted persona state, so a steward that comes up again reports what has moved since the last session rather than every already-unhealthy persona over again; a stored reading that lacks a persona entirely is the roster gaining one, and that is reported as new rather than as healthy. The first reading a persona ever makes compares against what each key looks like when nothing is wrong, so a fleet that is well when the steward first comes up says nothing while one already held is reported once.
...
Both fields live in the persisted persona state, and that state is a file inside a persona's own working directory, which the live roster gives to more than one persona. They are read back as a stranger's. A stored class is kept only where it is one the reduction produces, a stamp only where it is a finite number the clock has passed, and a suppression count only where it is not negative. A memo that fails any of those is dropped, and its key then reads as one the roster has just gained, so its next line is a move from `not in the previous reading`. Without that a hand-written memo silences the persona it names for good: a stamp in the far future makes every window comparison false, a window naming every class makes every change a flap, and a flap copies the stamp forward untouched, so the window never expires. The same shape in the cadence stamp stops the reconciliation pass from ever being asked for again.
