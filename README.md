...
- `.kit/stop-process-tree-test.sh` : `bin/supervise.sh`'s stop-path helpers and `stop_child` itself, against real Windows processes (not a `claude`-launching suite; run directly, not via `live-all.sh`)
