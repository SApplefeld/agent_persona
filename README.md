...
The two cases take different commands, and the hold marker is what separates them. A persona the roster added was never running, so it carries no marker and takes `Start-ScheduledTask AgentPersona-<name>` on its own. Registering a task does not start it: every `AgentPersona-*` task is triggered at startup (`bin/Register-PersonaTasks.ps1:303`), and the script starts one only under `-Start` (`:473-474`). Do not reach for `-Start` at the first step instead, because that would launch the new personas ahead of the settings deletion and the stops below, which is the order this procedure exists to get right. A persona you stopped through the shutdown tool gave the supervisor a clean exit 0, and the keeper turns exit 0 into a hold (`bin/keeper-functions.ps1:67-72`), so it stays down across every later boot until the marker goes: `Start-Persona.ps1 -Name <name> -Release` removes it, and `Start-ScheduledTask AgentPersona-<name>` then starts the persona again. A persona that went down with a reboot rather than a stop carries no marker and comes back on its own, so read `<rundir>/keeper.hold` to tell the two apart rather than assuming either.
