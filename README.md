...
When the owner's controller drains the inbox on a quiet tick, it submits the text through `$.prompt.submit` as an `[OPERATOR]` user turn. The `[OPERATOR]` marker is prepended to the text before submission. The delivery remembers which record it submitted, and the next `turn.start` stamps that turn's id onto that record only, and only when the turn is the plugin's own: not external (the real `prompt.submit` hook fired, which a keyboard, SDK or channel turn does and the plugin's own submits never do), not channel-origin, not opened by a nudge, and not opened by another of the plugin's own submits (the kaizen announcement, the reply backstop, the ask re-raise); a turn the plugin did not open for the record starting first is withheld (`operator_stamp_withheld`, naming `channel-origin`, `nudged`, `plugin` or `external`), and the record stays `delivered` with no reply, which the sender reads as unanswered rather than as a wrong answer. When the stamped turn completes with an answer, the controller writes that answer back as the reply and marks the record `answered`. A stamped turn that ends aborted or with an empty answer leaves the record `delivered` with its stamp (`operator_turn_unanswered`); no later turn re-stamps it, and the TTL bounds it.
