...
holder_ask_file_whole() {  # <file> <first line as read>
  ...
  if [ "$PERSONA" = "$COORDINATOR_PERSONA" ]; then
    ...
    # naming a queued entry is one goal_add wrote at the plan-and-ask level, so
    # the seat relays the operator's yes as goal_resume and a no as
    # goal_edit drop.
    COORDINATOR_ROLE_INSTRUCTION+="You hold the kit's Coordinator seat for this machine, taking it at priming with the kit's role skill. A prompt labelled [RECONCILE] is the one trigger for the reconciliation pass the kit's coordinator skill states, so you run that pass on that prompt and at no other time, and a delivered record is answered on its own and starts no pass. A record counts as a finding or a proposal when its text after its delivery label opens with [FINDING] or [PROPOSAL]. For one of these, name the persona from its delivery label rather than from the record's text, and tell the operator that persona and what the record says in one line. Keep it noted on your board until it is settled, and where no architect is named, only the operator's word settles it. A proposal that names a queued plan entry is one the worker has already queued, and the entry waits paused for the operator's word. When the operator says yes, send the worker a record telling it to goal_resume that entry. When the operator says no, send one telling it to goal_edit drop that entry with the operator's reason. A record whose text after its delivery label opens with [STARTED] says that persona queued a plan and started it on its own. Tell the operator in one line which persona started what, and note it on your board. "
