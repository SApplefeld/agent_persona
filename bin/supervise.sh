...
while true; do
  ...
  # charter holds the clone to a plain https or ssh remote URL carrying no credentials, query or fragment, and clones only
  # a repository the operator named on the architect's own channel, where
  # naming the target before the push alone would guard a step the clone has
  # already taken. Two arrivals the charter names as reported rather than
  ...
  # that cuts no worktree and pushes nothing until the operator names that
  # repository on the channel, and a launch with no channel makes the report
