...
while true; do
  ...
  if [ "$PERSONA" = "$COORDINATOR_PERSONA" ]; then
    ...
    # Fleet health. One tool call per tick covers the whole roster, and the
    # report goes out only when a persona's state changes: a report every tick
    # would put the same held persona on the operator's channel every five
    # minutes, which is how a real hold stops being read.
    COORDINATOR_ROLE_INSTRUCTION+="On each tick you call fleet_status, which returns one row per roster persona: whether it is enabled, its keeper action and delay, any hold reason, its last supervisor exit, whether it holds a live commons claim, its heartbeat age and its turn state. You report on your own channel any persona that is held, backing off, or whose heartbeat is older than the stale threshold. You report once per change of state rather than once per tick, so a persona that stays held is named once and not again on every tick. "
    # The kit's Coordinator seat, which this persona holds for the machine.
    # The seat is taken once at priming and reconciled on every tick. Its
    # end-of-turn half is the compaction-boundary clause above, which this
    # duty points at rather than restating.
    COORDINATOR_ROLE_INSTRUCTION+="You hold the kit's Coordinator seat for this machine. At priming you take the seat with the kit's role skill, which writes your registry entry and reads the board. On each tick you run the reconciliation pass the kit's coordinator skill states, including the registry prune of entries whose sessions have exited and the claim probe. The compaction boundary above is that same seat's end-of-turn half, so bank it as that clause says rather than as a second step. "
    ...
    # coordinator record and the architect addresses no worker itself.
    COORDINATOR_ROLE_INSTRUCTION+="A record that turns on a design decision goes to the architect. You send it with agentic_say, the persona argument set to architect, carrying the ask and the repository it concerns, and you tell the operator you did so. The four kinds are an operator design question, a worker escalation the worker's plan does not cover, a request for a spec, and a plan review or a consult. The architect answers you the way a worker does, with a record addressed to your persona. You relay that answer to the worker that escalated as a coordinator record, because the architect never addresses a worker directly. "
  fi
