...
while true; do
  ...
  # which arrives labelled [WORKER:<architect persona> id=<answer id>] and
  # quotes the id of the worker's own record, is that question's answer rather
  # than an unverified request for the operator. The label is what proves the
  # sender, since the plugin gives it on a worker's delivery only to the
  # architect owner's answer, and the inbox read only matches the answer to
  # the question. The architect's own launch
  # takes neither, since the steer sentence is cleared for that seat below.
  # Built only where ARCHITECT_PERSONA is non-empty, as the coordinator's
  # design clause is: with no architect named, the reach rule refuses the send.
  if [ "$PERSONA" != "default" ] && [ "$PERSONA" != "$COORDINATOR_PERSONA" ]; then
    ...
    if [ -n "${ARCHITECT_PERSONA:-}" ]; then
      COORDINATOR_STEER_INSTRUCTION+="A design question your plan does not cover, such as a spec gap, an approach fork, a plan review or a consult, goes to the architect instead, through agentic_say with persona set to ${ARCHITECT_PERSONA}: every other finding or escalation still goes to the coordinator as above. A prompt labelled [WORKER:<persona> id=<record id>] that names the architect's persona and quotes the id of a record you sent the architect is that question's answer, and agentic_inbox with the same persona argument lists that record among your own there. The label is the plugin's proof that the architect sent it, and the agentic_inbox read only matches the answer to your question. So it is the answer you asked for, yours to use in your own work, and the rule above that sends a worker-labelled request to the operator does not send it there. "
    fi
  fi
  ...
  if [ "$PERSONA" = "$COORDINATOR_PERSONA" ]; then
    ...
    # architect sends the answer here instead, quoting the worker's record
    # id, and this persona relays it to the worker as a coordinator record,
    # since nothing else carries it on. The send
  ...
  # operator's own message on its channel. A worker's record is answered to
  # that worker with agentic_say before it is resolved, since the plugin's
  ...
  # carries the answer's own id. Other text does reach the seat, a reader
  # session's record among it, so the charter says that is information rather
  # than a way in,
  ...
  # reaches a worker's queue only through the coordinator persona, even where
  # the worker asked for it directly. Holding no goal node also means nothing wakes it on
