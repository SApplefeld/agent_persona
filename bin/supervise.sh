...
if [ ! -f "$SETTINGS_FILE" ]; then
  ...
  if [ -n "$ARCHITECT_PERSONA_GIVEN" ] && [ "$ARCHITECT_PERSONA_GIVEN" != "$ARCHITECT_PERSONA_FILE" ]; then
    echo "$(date -u +%Y-%m-%dT%H:%M:%SZ) NOTE: launch environment carries ARCHITECT_PERSONA '$ARCHITECT_PERSONA_GIVEN' and $SETTINGS_FILE resolves architectPersona to '$ARCHITECT_PERSONA_FILE'; the launch environment wins" >> "$LOG"
  fi
...
while true; do
  ...
  # charter holds the clone to a plain https or ssh remote URL and clones only
  # a repository the operator named themselves, on the architect's own channel
  # or in the prompt its launch wrote, and reports rather than clones every
  # other arrival, a launch with no channel reporting to the steward. Where
  # naming the target before the push alone would guard a step the clone has
  # already taken. An ask that produces a file and
