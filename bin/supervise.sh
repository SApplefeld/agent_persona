...
while true; do
  ...
  # delivery, so the coordinator's status read sees them without asking in
  # a record. Appended for every launch but the coordinator's own, which
  # cannot address itself.
  if [ "$PERSONA" != "$COORDINATOR_PERSONA" ]; then
    COORDINATOR_STEER_INSTRUCTION+="A finding the coordinator should act on, and every coordinator steer you decline, also goes to the coordinator persona through agentic_say with persona set to ${COORDINATOR_PERSONA}, so its status read sees it without asking you in-band; what needs the operator's own decision still goes to the operator on your own channel. "
  fi
