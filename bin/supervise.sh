...
while true; do
  ...
  if [ -n "$PROMPT_FILE" ] && [ -f "$PROMPT_FILE" ]; then
    ...
    PRIMING_BODY="You are the passive supervisor. If a goal tree exists, resume it from goal_status, whether or not an entry is active; otherwise wait for a goal or a steering message from the operator. Reply now with one short line acknowledging you are ready, then carry on."
  else
    PRIMING_BODY="You are the passive supervisor. If a goal tree exists, resume it from goal_status, whether or not an entry is active; otherwise wait for a goal. No channel is attached, so no steering message arrives here. Reply now with one short line acknowledging you are ready, then resume or wait."
  fi
  if [ -n "$CHILD_IN" ]; then
    node -e "
