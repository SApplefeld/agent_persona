...
while true; do
  ...
  if [ "$PERSONA" = "$COORDINATOR_PERSONA" ]; then
    ...
    COORDINATOR_ROLE_INSTRUCTION+="A prompt labelled [FLEET] carries the personas whose health class changed since the last such prompt. You report each of those personas on your own channel, one line each, and you poll the fleet yourself at no point. The health classes are held, backing off, stale, no live claim while the roster enables it, and healthy. A persona the roster enables that holds no live claim has never come up, so it is down and you report it as down. You call fleet_status when the operator asks for fleet state, or when you need the whole picture behind a change. "
