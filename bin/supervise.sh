...
while true; do
  ...
  # than an unverified request for the operator. The architect's own launch
  ...
  if [ "$PERSONA" != "default" ] && [ "$PERSONA" != "$COORDINATOR_PERSONA" ]; then
    ...
    if [ -n "${ARCHITECT_PERSONA:-}" ]; then
      COORDINATOR_STEER_INSTRUCTION+="A design question your plan does not cover, such as a spec gap, an approach fork, a plan review or a consult, goes to the architect instead, through agentic_say with persona set to ${ARCHITECT_PERSONA}: every other finding or escalation still goes to the coordinator as above. A prompt labelled [WORKER:<persona> id=<record id>] that names the architect's persona and quotes the id of a record you sent the architect is that question's answer, and agentic_inbox with the same persona argument lists that record among your own there. So it is the answer you asked for, yours to use in your own work, and the rule above that sends a worker-labelled request to the operator does not send it there. "
    fi
  fi
  ...
  if [ "$PERSONA" = "$COORDINATOR_PERSONA" ]; then
    ...
    # worker directly, a line this clause does not govern. The send
