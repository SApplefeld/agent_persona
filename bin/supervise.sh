...
while true; do
  ...
  # delivery. The clause keys on a coordinator steer having reached the
  # worker, so a fleet with no coordinator launched sends nothing to a
  # persona nobody owns, where a pending record would sit unswept. Appended
  # for every named launch but the coordinator's own, which cannot address
  # itself; a default-persona launch holds no named owner claim, so the
  # reach rule would refuse its send and the clause is withheld.
  if [ -n "$PERSONA" ] && [ "$PERSONA" != "default" ] && [ "$PERSONA" != "$COORDINATOR_PERSONA" ]; then
    COORDINATOR_STEER_INSTRUCTION+="Once a coordinator steer has reached you, a finding it should act on, and every steer of its you decline, also goes to it through agentic_say with persona set to ${COORDINATOR_PERSONA}: that delivery wakes the coordinator, where a resolution alone waits for its next status read. What needs the operator's own decision still goes to the operator on your own channel. "
  fi
