...
while true; do
  ...
  if [ "$PERSONA" = "$COORDINATOR_PERSONA" ]; then
    ...
    # Fleet health. The plugin composes that prompt as a list of lines, one
    # per key of its reading, and quotes with a leading '> ' every line that
    # carries text out of a file rather than text it composed. The duty states
    # that rule, because a reader told to report each line and not told what a
    # quoted line is reports a persona's own forged words as a fleet event.
