...
while true; do
  ...
  # The channel-reply instruction below names the reply tool and points at
  # CLAUDE.md's "Writing to the operator" section, which owns how a reply is
  # written. It carries no copy of those rules: a second copy drifts from the
  # first, and the child reads CLAUDE.md at every launch anyway.
  ...
  if [ "$NO_CHANNEL" -ne 1 ]; then
    CHANNEL_REPLY_INSTRUCTION="You are attached to a Discord channel. Your own conversational reply never reaches the operator, so anything meant for them goes through the reply tool from the channel-relay MCP server. CLAUDE.md's 'Writing to the operator' section governs every message you send that way. "
  fi
  ...
  # goal-prompt framing above names that text the operator's trusted task. The
