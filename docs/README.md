...
- `plans/agent_persona_lean-injection_v1.md`: Lean injection (every prompt the plugin or supervisor writes into a session, and every tool description, says only what its reader cannot already know and points at the owner for the rest, with a duplicate-sentence test and a size ledger as the guard). In progress on the `lean-injection` branch, ahead of the supervisor-peer plan in the running order. Both plans it waited on have closed: the steward-architect plan merged as pull request 49, and the context-budget-removal plan is archived.
...
- `archive/agent_persona_context-budget-removal_v1.md`: Context-budget monitor removal (the plugin no longer estimates how full a session's context window is, logs no crossing and writes no close-out turn into the conversation, and the supervisor no longer restarts a child on that estimate; context is left to the harness's own compaction and the kit's gate, and the coordinator's compaction-boundary sentence in the priming turn stays). Complete; the most recent plan. Its deferred suites run under `plans/agent_persona_deferred-gate-run_v1.md`, and updating the installed plugin copy and relaunching each supervisor is the operator's own act, listed under its Operator Verification.
