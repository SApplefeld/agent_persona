...
`.kit/injection-ledger.json` is the committed size baseline, 40 entries totalling 33,144 characters. Ten entries come from `bin/supervise.sh` and total 16,226; thirty come from `hooks/index.ts` and total 16,918, of which the fifteen registered tool descriptions are 12,313.

| What a launch reads | Characters |
|---|---|
| A worker with a channel: skill-load, coordinator steer, reply-tool | 3,665 |
| The coordinator: those three plus the coordinator role instruction | 9,041 |
