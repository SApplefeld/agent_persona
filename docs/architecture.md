...
2. The task's action runs `powershell.exe -NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -File <repo>/bin/Start-Persona.ps1 -Name <name> -Roster <roster> -EnvFile <envfile>`, every path absolute (`bin/Register-PersonaTasks.ps1:298-300`).
...
`.kit/injection-ledger.json` is the committed size baseline, 40 entries totalling 29,525 characters. Ten entries come from `bin/supervise.sh` and total 13,480; thirty come from `hooks/index.ts` and total 16,045, of which the fifteen registered tool descriptions are 11,635.
...
| The fifteen tool descriptions an owner-tier session registers | 11,635 |
