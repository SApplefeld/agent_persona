...
## The tree-lag self-review signal counts ordinary section commits as a defect (found 2026-09-23)

`collectEvents` in `hooks/self-review.ts` records a `tree_lag` event on every `env_git` sample that reads `dirty=0` after a dirty read, where no goal-tree write (`create`, `add`, `done`, `complete`, `completed_by_controller`, `drop`) landed since the previous cleared sample. A `plan` node stands for one plan document, and a worker under the kit lands one commit per section and often a plan-document commit beside it, so two consecutive commits with no node change are the normal shape of the work and not a lag. The node the signal writes asks the controller to close "the node that a commit closes" within a tick, and no commit names a node, so the remedy it proposes cannot be built; the controller already closes a plan node when its document reads Complete, under `completed_by_controller` in `hooks/index.ts`. The dev-plugin persona's store holds this node as `plan-mub7lein-1sk9`, paused, with four scored rounds that all read `drift` or `off-goal-by-instruction`, which is what a goal against a false reading produces. The goal-levels plan routes that node out as a finding and sets it `abandoned`; it does not touch the signal, so the signal writes the same finding again on the next two commits. Remedy: retire the `tree_lag` signal, its `describe` branch and its `TREE_WRITE_ACTIONS` set, and the `KaizenSignal` union with them, or rescope it to what is real, a plan document reading Complete on origin whose node is still open past one tick, which the controller already handles, so retirement is the recommendation. Proof: a self-review unit case where a decision log holding two `dirty=0` samples and no tree write yields no finding, and the tick suite and the self-review unit suite green.

_Retired 2026-09-23: done by `docs/plans/agent_persona_kaizen-signals_v1.md`, which removes the `tree_lag` signal from `hooks/self-review.ts` (branch kaizen-signals-build, commits d3dc56f and 9886522)._

## The long-turns self-review signal proposes a bounded turn the design refuses (found 2026-09-23)

`turn.complete` in `hooks/index.ts` records `turn_over_hour` for a turn that ran sixty minutes or longer, and `reviewOwnRecord` in `hooks/self-review.ts` turns two of them into a `long_turns` finding. The finding's configuration half is sound and stays: a review cadence counted in turns runs too rarely when turns run for hours, so halving `selfReviewEveryTurns` is the right answer. Its goal half is not. Once the cadence is at the floor the finding becomes a goal whose objective is to "split the work into turns the operator can steer between", with a harness case injecting a bounded turn plan once a turn runs long. A long turn is the kit's own design: executing-work runs a plan to completion, and capacity is never a blocker. The steer the objective asks for already exists inside a turn rather than between turns: an `urgent` record, and any record aged past `breakInAfterMs`, breaks into the running turn as context on the next tool result, and the channel relay delivers the operator's own message mid-turn, deferred to the turn boundary by doctrine. The dev-plugin persona's store holds this node as `plan-mub7lein-l8ku`, paused, with two sample turns of 12729 and 12803 seconds. The goal-levels plan routes that node out as a finding and sets it `abandoned`, and leaves the signal to write the same goal again. Whether a bounded turn is wanted at all is the operator's call, and this entry recommends against: drop the goal half of `long_turns`, keep the configuration half, and let the signal yield nothing once the cadence sits at the floor. Proof: a self-review unit case where a `long_turns` finding with the cadence at the floor yields no goal-shaped finding, and the same input above the floor still yields the configuration fix.

_Retired 2026-09-23: done by `docs/plans/agent_persona_kaizen-signals_v1.md`, which keeps the cadence half of `long_turns` and drops its goal half, so the signal yields nothing at the floor (branch kaizen-signals-build, commits d3dc56f and 9886522)._
