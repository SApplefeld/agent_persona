...
`listInboxRecords` in `hooks/operator.ts` pushes each store value through as a record with no string parse, while every consumer that re-reads the same key parses a string value first. A record persisted as a JSON string therefore reads with `status` undefined, and is dropped in silence by the break-in scan's pending filter and by the reply backfill in `turn.complete`. The break-in leans on that list harder than the urgent-only path did, so the silent drop reaches more records. Remedy: parse a string value in `listInboxRecords` the way `readReplyRecord` already does, with a case that seeds a string-persisted record and fails before the parse is added. It is kept separate because that function has many consumers and the change deserves its own red-then-green rather than riding along with the break-in work.
