...
## The two running supervisors still carry the pre-PR-#17 one-pid stop path

The runtime clone at `/d/DeepSeekHarness/agentic-plugin` is current at `0fc66d2`, which carries PR #17, but a running `bin/supervise.sh` keeps reading its original open file handle and never picks up a pull. Both supervisors live on this machine were launched before that pull, so their `stop_child` still signals one pid rather than the whole Windows process tree, and a restart taken through either still hits the `GATE TIMEOUT` PR #17 removes. Nothing to fix in the tree: relaunch both supervisors, then drop this entry. This is the narrowed remainder of the stale-clone entry retired in item 3's runtime-clone addendum.
