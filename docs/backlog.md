...
`run_bounded_powershell` in `bin/supervise.sh` runs `taskkill //F //PID "$ps_real_pid"` at line 647 when its bound expires, where `$ps_real_pid` is the pid `powershell.exe` wrote to its own output file at start. Nothing checks that the process is still alive or start-time matched before the kill. In the three-process shape the comment at lines 593 to 599 describes, `powershell.exe` can exit while the MSYS stub lingers, so the pid can be recycled inside that window and the kill lands on whatever now holds it. Every other kill in the file is guarded: the snapshot kills are pid-and-ticks matched at lines 1137 to 1141, the stub kill at 651 fires only while `kill -0` holds on the MSYS process that owns the winpid, and the wrapper kill at 2071 re-resolves and compares first. This predates the process keeper plan's base ref, so it was left out of that changeset rather than found and ignored. Remedy: record a ticks value alongside the pid the PSPID line carries, and gate the taskkill on a `Get-Process -Id` StartTime read against it, or drop the real-pid kill and rely on the stub kill alone. Proof: a case where the recorded pid belongs to a process started after the bound opened, asserting no kill is issued.

## A child's session id reaches a filesystem path unsanitized (found 2026-09-17)

`read_child_session_id` in `bin/supervise.sh` returns the `session_id` field verbatim from the child's own `stdout.jsonl`, and `read_transcript_mtime_ms` splices it into `$profile_u/.claude/projects/$key/$session_id.jsonl` at line 2371. The workdir half of that path is sanitized one line earlier, replacing every non-alphanumeric character with a hyphen; the session id is not, so a value carrying `../` reaches any `.jsonl` file under the profile for an mtime read. The impact is bounded and no privilege is gained: the read is a modification time, and a child that wants to defeat its own hung-restart already controls the heartbeat that check reads. This is defense in depth on an input that crosses a trust boundary from the child, not a live hole. It was kept out of the process keeper plan's closing changeset because `bin/supervise.sh` is one of the two files `.kit/supervisor-natural-exit-test.sh` drives, and that suite costs over an hour per run. Remedy: accept the session id only when it matches a UUID shape, and treat anything else as no transcript. Proof: a unit case feeding a session id carrying `../`, asserting the composed path is refused rather than read.

## The registration script accepts a roster entry the wrapper cannot launch (found 2026-09-17)

`Get-PersonaTaskDefinitions` in `bin/Register-PersonaTasks.ps1` refuses a missing env file at line 236, on the reasoning its own comment gives at lines 186 to 188: the task would otherwise fail to start at boot with nothing to say why until then. It does not apply that reasoning to the fields the wrapper itself needs. `Build-SupervisorInvocation` in `bin/keeper-functions.ps1` throws at lines 216 to 221 when an entry is missing `name`, `workdir` or `permissionMode`, and it runs at boot rather than at registration. So an entry with a run directory set and `permissionMode` absent registers cleanly, and then the wrapper exits 1 on every launch while the scheduler retries 999 times at one minute, writing an error line each time. The shape behind it is two roster readers with different rules: `Read-PersonaRoster` validates for registration and `Read-KeeperRoster` validates for the launch, and neither shares a check with the other. Remedy: have the definition builder call `Build-SupervisorInvocation` for each enabled entry, so a roster the wrapper cannot launch is refused where an operator is watching. Proof: a register case whose fixture entry omits `permissionMode`, red before the change and refused at registration after it.

## The natural-exit suite's wall clock is set by real-time waits, and it now costs over an hour (found 2026-09-17)

`.kit/supervisor-natural-exit-test.sh` took 109 minutes on 2026-09-17, measured from its output file's first write to its exit marker, against an operator's recollection of about sixteen minutes earlier in this program's life. The cost is not case count. The suite drives the real `bin/supervise.sh` against a stub child and waits real seconds rather than on a signal: two cases set `SUPERVISOR_POLL_MS=10000` (lines 376 and 620), so every poll those cases need costs ten seconds of wall clock, and the stub child and the injections carry `sleep 60` (line 1318), `sleep 33` (line 1805), `sleep 26` (line 1341), `sleep 11` (line 1276) and `sleep 8` (line 1707). The suite's only bound is one `timeout 420` on the driver at line 1385, which is per driven run rather than per suite, so nothing caps the total. Two consequences fall out of this beyond the wait itself. A run this long cannot survive a session that is killed or restarted mid-suite, which is why this plan's gate repeatedly failed to complete. And a fixed real-time wait is exactly what a loaded box blows: the same run returned `rc=124` at that 420-second bound and 14 failures across five case families, none of which reproduce on an idle box. Remedy: time each case first and find the handful that dominate, then replace their fixed waits with a wait on the signal each case is actually waiting for (a log line, a marker file, a heartbeat write), and lower `SUPERVISOR_POLL_MS` in the cases that only need a poll to have happened rather than to have taken ten seconds. Proof: the suite's total wall clock recorded before and after, and every case still red under a mutation that removes the behavior it pins.

## The shipped example roster has no test pinning it to what the registration script accepts (found 2026-09-17)

`bin/fleet.example.json` is the roster an operator copies to `D:/personas/fleet.json`. Nothing in the suites reads it: `.kit/keeper-register-test.mjs:82` builds its cases from its own `.kit/fixtures/fleet.fixture.json`, and `.kit/keeper-unit-test.mjs` writes scratch rosters of its own. So a field renamed in `bin/keeper-functions.ps1`, or an entry edited in the example, leaves both suites green while the shipped example stops being a roster the scripts can read. The check that caught it today was a hand-run `Register-PersonaTasks.ps1 -WhatIf`, which no gate repeats. This is a writer and a reader tested only against their own literals, which is the shape a cross-component pin exists for. Remedy: add one case to `.kit/keeper-register-test.mjs` that runs the definition builder against `bin/fleet.example.json` itself and asserts one definition per enabled entry. Proof: the case is red when a field in the example is renamed and green on the file as it ships.
