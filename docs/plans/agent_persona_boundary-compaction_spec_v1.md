...
Files in scope: `hooks/index.ts` (the `turn.complete` handler at ~6570, its `openTurns`/`currentGateTurnId` boundary check, and the `readLeadLine`, `planHolderOf` and plan-record read it already computes), `.kit/controller-tick-test.mjs`, `.kit/tick-harness.mjs`.
