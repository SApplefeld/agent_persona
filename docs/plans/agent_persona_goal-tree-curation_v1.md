...
Next action. Hold for the adversarial reviewer, then adjudicate the round whole. The findings interact, and one of them bears on where the guard is placed rather than only on its call sites, so a fix dispatched against half the round risks a rewrite. The honesty question on the composed sentence is the one to settle first, since a check that pins an untrue operator sentence cannot close a section.
