...
Next action. The fix is dispatched on the throw shape, with the Section 6 heal case amended where it asserts what the Goal now forbids, and one new check pinning the observable that separates a throw from a false return. Then re-run the targeted lane against the bd192a4 baseline and take review round 2.
