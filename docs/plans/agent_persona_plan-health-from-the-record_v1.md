...
Next action per section: read the resumed implementer's report, verify it on this session's own run, commit the follow-up, then dispatch review round 1 at round 1's roster one tier up from the writer (fable for the code pair, plus the security lens since the section reads a file at a path the store supplies).
