...
| **`buildSelfReviewInput(state, now)`** | `hooks/self-review.ts` | Builds the input window: filtered decisions (exclude `self-review`, `controller_tick`, `env_inject`, `heartbeat`), `env.errors`, active goal node, live self-review lessons. Returns `{ prompt: string; decisionTimestamps: number[]; turnRange: [number, number]; streak: number }`. |
