...
| **Lesson injection** | `index.ts:2280-2297` (the `[ENV]` block area, in the `prompt.submit` handler) | After the `[ENV]` block: filter `source === "self-review" && kind === "lesson"`, sort by `createdAt` descending, take the newest. If `newest.createdAt > sr.lastInjectAt`, push `[LESSON] ${newest.text}\nA self-review lesson from recent activity. Avoid repeating the same mistake.` to `contextBlocks`, push `lesson_inject` decision, set `sr.lastInjectAt`. Only one lesson is injected (the newest); a second lesson is rarely newer than `lastInjectAt` (U5). |
