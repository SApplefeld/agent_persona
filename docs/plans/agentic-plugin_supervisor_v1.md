...
| AD1 | Restarted children never own the persona; `persist()` vetoes the claimant who just overtook a dead holder | `writeClaimDirect(dp)` helper in `hooks/index.ts` writes `sess.state` to `sess.storePath` directly + heartbeat entry at both claim sites (session.start and heartbeat tick) before `persist()`; `persona_claim` detail includes new session id; folded into commit `1b02370` + `6946c9c` |
