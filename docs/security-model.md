...
- The credentials the operator's account can use. Some are protected by Windows' per-account encryption key, DPAPI, which any process running as the account can use: every Windows Credential Manager entry and Git Credential Manager's store. Others sit in files in the account's profile, readable by any process running as the account: the fleet's GitHub token in `.config/gh/hosts.yml`, which git uses for every push through `gh auth git-credential`, an SSH key in `.ssh`, and Claude Code's own `.claude/.credentials.json`.
...
- A party holding SYSTEM, the Windows service account above every user. The scheduler keeps each task's password in the LSA vault, a system store that SYSTEM can read by the platform's design.
...
**Every child holds the account's whole credential reach.** A task that logs on with a stored password unlocks the account's DPAPI key for everything the task runs (`bin/Register-PersonaTasks.ps1`, the comment above `New-ScheduledTaskPrincipal`). Every child can also read the credential files in the profile. So every persona can use every credential listed under "Assets", with nothing bounding what it does with them. Because SYSTEM is out of consideration, this model also accepts that a party holding it can recover the password from the LSA vault. This risk rests on the same four preconditions as the one above.
